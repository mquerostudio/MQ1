 ...
 *
 * This function turns on the seesaw device by setting the specified pin as input, enabling pull-up, and setting the pin to a high state.
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @return
 ...
 *
 * This function sets the specified seesaw device pin as an output and sets it to a low state.
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @return
 ...
 */
esp_err_t seesaw_pin_mode(const seesaw_handle_t seesaw_handle, uint8_t pin, uint8_t mode)
{
 ...
 *
 * This function sets the pin mode for multiple pins on a seesaw device. The pins parameter is a bitmask
 * where each bit represents a pin. The mode parameter specifies the desired pin mode, which can be one of
 * the following:
 ...
 * @return esp_err_t Returns ESP_OK if the operation is successful, otherwise an error code.
 */
esp_err_t seesaw_pin_mode_bulk(const seesaw_handle_t seesaw_handle, uint32_t pins, uint8_t mode)
{
    esp_err_t ret = ESP_OK;
    uint8_t cmd[] = {(uint8_t)(pins >> 24), (uint8_t)(pins >> 16), (uint8_t)(pins >> 8), (uint8_t)pins};
    ...
    switch (mode) {
        case OUTPUT:
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_DIRSET_BULK, cmd, 4);
            break;
        case INPUT:
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_DIRCLR_BULK, cmd, 4);
            break;
        case INPUT_PULLUP:
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_DIRCLR_BULK, cmd, 4);
            if (ret != ESP_OK) {
            ...
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_PULLENSET, cmd, 4);
            if (ret != ESP_OK) {
                break;
            }
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_BULK_SET, cmd, 4);
            if (ret != ESP_OK) {
                break;
            }
            break;
        case INPUT_PULLDOWN:
            ...
            ret = seesaw_write(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_BULK_CLR, cmd, 4);
            if (ret != ESP_OK) {
                break;
            }
            break;
 ...
 *
 * This function writes a digital value to the specified pin on the seesaw device.
 *
 * @param seesaw_handle The handle to the seesaw device.
 ...
 * @return `ESP_OK` on success, or an error code if an error occurred.
 */
esp_err_t seesaw_digital_write(const seesaw_handle_t seesaw_handle, uint8_t pin, uint8_t value)
{
 ...
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @param pins The pins to write to. Each bit represents a pin, where a set bit indicates the corresponding pin should be written to.
 * @param value The value to write to the pins. A non-zero value sets the pins to high, while a zero value sets the pins to low.
 *
 * @return
 *     - ESP_OK: Success
 *     - Error code: Fail
 */
esp_err_t seesaw_digital_write_bulk(const seesaw_handle_t seesaw_handle, uint32_t pins, uint8_t value)
{
    esp_err_t ret = ESP_OK;
    uint8_t cmd[] = {(uint8_t)(pins >> 24), (uint8_t)(pins >> 16), (uint8_t)(pins >> 8), (uint8_t)pins};
 ...
 *
 * This function reads the digital value of the specified pin on the seesaw device.
 *
 * @param seesaw_handle The handle to the seesaw device.
 ...
 *
 * This function reads the digital values of multiple pins in bulk from the seesaw device.
 *
 * @param seesaw_handle The handle to the seesaw device.
 ...
 */
uint32_t seesaw_digital_read_bulk(const seesaw_handle_t seesaw_handle, uint32_t pins)
{
    esp_err_t ret = ESP_OK;
    uint8_t buf[4];

    ret = seesaw_read(seesaw_handle->i2c_dev, SEESAW_GPIO_BASE, SEESAW_GPIO_BULK, buf, 4);
    if(ret != ESP_OK)
    {
    ...
    uint32_t ret1 = ((uint32_t)buf[0] << 24) | ((uint32_t)buf[1] << 16) | ((uint32_t)buf[2] << 8) | (uint32_t)buf[3];
 ...
 */
esp_err_t seesaw_encoder_set_position(const seesaw_handle_t seesaw_handle, int32_t pos)
{
...
/**
 * @brief Writes an 8-bit value to the specified register address of the seesaw device.
 *
 * This function writes an 8-bit value to the specified register address of the seesaw device
 ...
 *
 * This function writes data to the seesaw device using the specified I2C master device handle.
 * The data to be written is provided in the `buf` array, with the number of bytes specified by `num`.
 * The register address to write to is specified by `regHigh` and `regLow`.
 *
 * @param i2c_dev The I2C master device handle.
 * @param regHigh The high byte of the register address.
 * @param regLow The low byte of the register address.
 ...
 *
 * This function reads data from the seesaw device using the specified I2C master device handle.
 *
 * @param i2c_dev The I2C master device handle.
 * @param regHigh The high byte of the register address.
 * @param regLow The low byte of the register address.
 ...
 *
 * This function initializes the NeoPixel module of the SeeSaw component with the provided configuration.
 *
...
esp_err_t seesaw_neopixel_init(const seesaw_handle_t seesaw_handle, const neopixel_config_t *neopixel_config)
{
    ...
    ret = seesaw_neopixel_update_type(seesaw_handle, seesaw_handle->neopixel_handle->type);
    if (ret != ESP_OK)
    {
    ...
    ret = seesaw_neopixel_update_length(seesaw_handle, seesaw_handle->neopixel_handle->numLEDs);
    if (ret != ESP_OK)
    {
    ...
    ret = seesaw_neopixel_set_pin(seesaw_handle, seesaw_handle->neopixel_handle->pin);
    if (ret != ESP_OK)
    {
 ...
 * 
 * @return `ESP_OK` if the neopixel type is updated successfully, otherwise an error code.
 */
esp_err_t seesaw_neopixel_update_type(const seesaw_handle_t seesaw_handle, uint16_t t)
{
    ...
    if (seesaw_handle->neopixel_handle->pixels) {
      ...
      if (newThreeBytesPerPixel != oldTreeBytesPerPixel)
        seesaw_neopixel_update_length(seesaw_handle, seesaw_handle->neopixel_handle->numLEDs);
    }
 ...
 *
 * This function updates the length of the NeoPixel strip by allocating new memory for the pixels.
 * If there is existing data, it will be freed before allocating new memory.
 * All pixels are cleared after allocating new memory.
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @param n The new length of the NeoPixel strip.
 * @return esp_err_t Returns ESP_OK if the length update is successful, otherwise an error code.
 */
esp_err_t seesaw_neopixel_update_length(const seesaw_handle_t seesaw_handle, uint16_t n)
{
    if (seesaw_handle->neopixel_handle->pixels)
 ...
 *
 * This function sets the pin for the NeoPixel on the seesaw device specified by `seesaw_handle`.
 *
 ...
 */
esp_err_t seesaw_neopixel_set_pin(const seesaw_handle_t seesaw_handle, uint8_t p)
{
    esp_err_t ret = ESP_OK;
 ...
 *
 * This function checks if the neopixel can be shown based on the time elapsed since the last show.
 *
 * @param seesaw_handle The handle to the seesaw component.
 ...
 *
 * This function sends the neopixel data stored in the seesaw handle to the seesaw device for display.
 * If there is no pixel data to send, the function returns without performing any action.
 * The function waits until the seesaw device is ready to receive the data before sending it.
 * After sending the data, the function saves the end time for the latch on the next call.
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @return `ESP_OK` if the neopixel data is successfully sent, or an error code if an error occurred.
 */
 ...
 */
void seesaw_neopixel_set_brightness(const seesaw_handle_t seesaw_handle, uint8_t b)
{
 ...
 *
 * This function takes the red, green, and blue components as input and combines them into a single 32-bit color value.
 *
...
/**
 * @brief Returns the number of pixels in the neopixel strip connected to the seesaw device.
 *
 * @param seesaw_handle The handle to the seesaw device.
...
/**
 * @brief Sets the color of a specific pixel in the NeoPixel strip connected to the seesaw device.
 *
 * This function sets the color of the specified pixel in the NeoPixel strip connected to the seesaw device.
 * The pixel index `n` should be within the range of available pixels on the strip.
 * The color `c` should be a 32-bit RGB color value, where the most significant byte represents the red component,
 * the middle byte represents the green component, and the least significant byte represents the blue component.
 * If the brightness of the NeoPixel strip is non-zero, the color values will be scaled accordingly.
 * The function calculates the appropriate offset based on the configuration of the NeoPixel strip and sets the
 * corresponding color values in the pixel buffer.
 * Finally, it writes the updated pixel buffer to the seesaw device via I2C communication.
 *
 * @param seesaw_handle The handle to the seesaw device.
 * @param n The index of the pixel to set the color for.
 * @param c The 32-bit RGB color value to set for the pixel.
 * @return `ESP_OK` if the operation is successful, otherwise an error code indicating the cause of failure.
 */
esp_err_t seesaw_neopixel_set_pixel_color(const seesaw_handle_t seesaw_handle, uint16_t n, uint32_t c)
{   
    esp_err_t ret = ESP_OK;
...
uint32_t seesaw_neopixel_get_pixel_color(const seesaw_handle_t seesaw_handle, uint16_t n)
{
    if(n >= seesaw_neopixel_num_pixels(seesaw_handle))
    {
        return 0;
    ...
    if(seesaw_handle->neopixel_handle->wOffset == seesaw_handle->neopixel_handle->rOffset) //Is RGB-type device
    {
        p = &seesaw_handle->neopixel_handle->pixels[n * 3];
        ...
        if(seesaw_handle->neopixel_handle->brightness)
        {
            ...
            return (((uint32_t)(p[seesaw_handle->neopixel_handle->rOffset] << 8) / seesaw_handle->neopixel_handle->brightness) << 16) |
                   (((uint32_t)(p[seesaw_handle->neopixel_handle->gOffset] << 8) / seesaw_handle->neopixel_handle->brightness) << 8) |
                   ((uint32_t)(p[seesaw_handle->neopixel_handle->bOffset] << 8) / seesaw_handle->neopixel_handle->brightness);
        } else
        {
            // No brightness adjustment has been made -- return 'raw' color
            return ((uint32_t)p[seesaw_handle->neopixel_handle->rOffset] << 16) | ((uint32_t)p[seesaw_handle->neopixel_handle->gOffset] << 8) | (uint32_t)p[seesaw_handle->neopixel_handle->bOffset];
        }   
    } else // Is RGBW-type device
    {
        ...
        {
            // Return scaled color
            return (((uint32_t)(p[seesaw_handle->neopixel_handle->wOffset] << 8) / seesaw_handle->neopixel_handle->brightness) << 24) |
                   (((uint32_t)(p[seesaw_handle->neopixel_handle->rOffset] << 8) / seesaw_handle->neopixel_handle->brightness) << 16) |
                   (((uint32_t)(p[seesaw_handle->neopixel_handle->gOffset] << 8) / seesaw_handle->neopixel_handle->brightness) << 8) |
                   ((uint32_t)(p[seesaw_handle->neopixel_handle->bOffset] << 8) / seesaw_handle->neopixel_handle->brightness);
        } else
        {
            // Return raw color
            return ((uint32_t)p[seesaw_handle->neopixel_handle->wOffset] << 24) | ((uint32_t)p[seesaw_handle->neopixel_handle->rOffset] << 16) | ((uint32_t)p[seesaw_handle->neopixel_handle->gOffset] << 8) | (uint32_t)p[seesaw_handle->neopixel_handle->bOffset];
        }
    }   
}